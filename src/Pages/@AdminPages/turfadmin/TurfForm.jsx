...
function TurfForm() {
  ...
  const today = new Date()
    ...
    .split(",")[0];
  const [footballPrices, setFootballPrices] = useState({
    "5's": "",
    "7's": "",
  });
    ...
    const newTurfData = {
      ...
      sports: sports.map((sport) => ({
        ...
        prices:
          sport === "football"
            ? {
                fiveAside: footballPrices["5's"],
                sevenAside: footballPrices["7's"],
              }
  ...
  const sendTurfData = async (turfData) => {
    if (
      turfData.sports.length == 0 ||
      !turfData.location ||
      turfData.amenities.length == 0
    ) {
    ...
    if (token) {
      ...
      try {
        ...
        if (result.status === 200) {
          ...
          setFootballPrices({
            "5's":
              data?.sports.find((s) => s.name === "football")?.prices[
                "fiveAside"
              ] || "",
            "7's":
              data?.sports.find((s) => s.name === "football")?.prices[
  ...
  const handleStatus = async () => {
    ...
    if (token) {
      ...
      try {
        ...
        const result = await changeStatusAPI(
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      ...
      {Object.keys(fetchTurfData).length > 0 ? (
        <>
          <h1 className="text-center mt-2 fw-bolder text-light p-4 fs-1 border bg-success">
            {fetchTurfData.name}
          </h1>
          <Container
            className="mt-4 p-0 fluid"
            style={{
                ...
                <Form>
                  ...
                  <div className="d-flex justify-content-center align-items-center mt-4">
                    <h3 className="fw-bolder fs-1">
                      {isChecked ? "You are Open" : "You are Closed"}
                    </h3>
                    <Form.Check
                      type="switch"
                      checked={isChecked}
      ...
      >
        <Card>
          <Card.Header as="h4" className="text-center bg-success text-light">
            {Object.keys(fetchTurfData).length > 0 ? "Edit" : "Add"} Your Arena{" "}
            <button
                          ...
                          <Col xs={6}>
                            {sports?.includes(sport) && (
                              <Form.Control
                                ...
                                value={prices?.[sport] || ""}
                                onChange={(e) =>
                                  handlePriceChange(sport, e.target.value)
                                }
                              />
                            )}
                          </Col>
